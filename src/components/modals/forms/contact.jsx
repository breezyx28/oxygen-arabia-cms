import * as React from 'react';
import Button from '@mui/material/Button';
import { styled } from '@mui/material/styles';
import Dialog from '@mui/material/Dialog';
import DialogTitle from '@mui/material/DialogTitle';
import DialogContent from '@mui/material/DialogContent';
import DialogActions from '@mui/material/DialogActions';
import IconButton from '@mui/material/IconButton';
import Iconify from 'src/components/iconify';
import { Stack, TextField, MenuItem, Select, InputLabel, FormControl, Box, Grid } from '@mui/material';
import InputErrorAttributes from 'src/components/utils/InputErrorAttributes';
import { ContactValidation } from 'src/logic/Validation/contactValidation';
import { useCreateContactMutation } from 'src/redux/endpoints/contacts';
import convertToFormData from 'src/helper/ConvertToFormData';
import * as MuiIcons from '@mui/icons-material'; // Import all MUI icons
import FileUploadInput from 'src/components/Inputs/FileUploadInput';

const BootstrapDialog = styled(Dialog)(({ theme }) => ({
  '& .MuiDialogContent-root': {
    padding: theme.spacing(2),
  },
  '& .MuiDialogActions-root': {
    padding: theme.spacing(1),
  },
}));

export default function ContactModal() {
  const [open, setOpen] = React.useState(false);

  const [contact, { data, isLoading, isSuccess, error, isError }] = useCreateContactMutation();

  const { controller, displayError } = new ContactValidation();
  const {
    register,
    handleSubmit,
    reset,
    setValue,
    formState: { errors: yupErrors },
  } = controller();

  const onSubmit = async (values) => {
    const converted = convertToFormData(values);
    contact(converted);
  };

  const handleClickOpen = () => {
    setOpen(true);
  };

  const handleClose = () => {
    setOpen(false);
  };


  return (
    <React.Fragment>
      <Button
        variant="contained"
        color="inherit"
        startIcon={<Iconify icon="eva:plus-fill" />}
        onClick={handleClickOpen}
      >
        Create Contact
      </Button>
      <BootstrapDialog onClose={handleClose} aria-labelledby="customized-dialog-title" open={open} fullWidth>
        <DialogTitle sx={{ m: 0, p: 2 }} id="customized-dialog-title">
          Create contact Section
        </DialogTitle>
        <IconButton
          aria-label="close"
          onClick={handleClose}
          sx={{
            position: 'absolute',
            right: 8,
            top: 8,
            color: (theme) => theme.palette.grey[500],
          }}
        >
          <CloseIcon />
        </IconButton>
        <DialogContent dividers sx={{ width: `100%` }}>
          <form method="POST" onSubmit={handleSubmit(onSubmit)} style={{ width: `100%` }}>
            <Stack spacing={3}>

              <Box sx={{ flexGrow: 1, gap: 4 }}>

                <Grid container spacing={1} justifyContent={'center'} alignItems="center">
                  <FileUploadInput
                    label={'Upload Contact Icon'}
                    name={'icon'}
                    title={'Contact Icon'}
                    setValue={setValue}
                    error={{
                      ...InputErrorAttributes({ inputName: 'icon', yupError: yupErrors }),
                    }}
                  />
                </Grid>
              </Box>
              <TextField
                fullWidth
                {...register('name')}
                label="Name"
                {...InputErrorAttributes({ inputName: 'name', yupError: yupErrors })}
              />
              <TextField
                fullWidth
                type="value"
                {...register('value')}
                label="Value"
                {...InputErrorAttributes({ inputName: 'value', yupError: yupErrors })}
              />
            </Stack>
            <Button
              type="submit"
              id="contact-form"
              style={{ display: 'none' }}
              autoFocus
            >
              Submit
            </Button>
          </form>
        </DialogContent>
        <DialogActions>
          <Button
            autoFocus
            onClick={() => {
              document.getElementById('contact-form').click();
            }}
          >
            Save changes
          </Button>
        </DialogActions>
      </BootstrapDialog>
    </React.Fragment>
  );
}

const CloseIcon = () => (
  <svg
    width={24}
    height={24}
    fill="none"
    stroke="#697686"
    strokeLinecap="round"
    strokeLinejoin="round"
    strokeWidth={2}
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="m5 5 14 14M5 19 19 5" />
  </svg>
);