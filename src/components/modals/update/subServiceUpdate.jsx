...
import { useGetSubserviceQuery, useUpdateSubserviceMutation } from 'src/redux/endpoints/sub-services';
...
export default function SubserviceUpdateModal({ id }) {
    const [open, setOpen] = React.useState(false);
    const [selectService, setSelectService] = React.useState('');

    const [updateSubservice, { data, isLoading, isSuccess, error, isError }] = useUpdateSubserviceMutation();

    const { data: subServices, isLoading: subServicesLoading, refetch } = useGetSubserviceQuery({
        id
    })
    ...
    const onSubmit = async (values) => {
        ...
        updateSubservice({ id, body: converted });
    };
    ...
    React.useEffect(() => {
        if (subServices) {
            ...
            setValue('image', '')
            setValue('card_image', '')
            setSelectService(subServices?.service?.id)
        }
    }, [subServices])
                        ...
                        <Stack spacing={3}>
                            ...
                            <Box sx={{ flexGrow: 1, gap: 4 }}>
                                ...
                                <Grid container spacing={1} justifyContent={'center'} alignItems="center">
                                    {subServices?.image && <FileUploadInput
                            ...
                            <Box sx={{ flexGrow: 1, gap: 4 }}>
                                ...
                                <Grid container spacing={1} justifyContent={'center'} alignItems="center">
                                    {subServices?.card_image && <FileUploadInput
                                        label={'Card Image'}
                                        name={'card_image'}
                                        title={'Card Image'}
                                        setValue={setValue}
                                        ...
                                        showUrl={subServices?.card_image}
                                    />}
                                </Grid>
                            </Box>
                        </Stack>
